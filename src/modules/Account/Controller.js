  ...
  async login(req, res) {
    try {
      ...
      if (_check == false && req.account) {
        ...
        console.log(bcrypt.compareSync(password, req.account.password),bcrypt.hashSync('123456', bcrypt.genSaltSync(12)))
       if (
    ...
    return new Promise(async(resolve, reject) => {
      ...
      if(role_id != NaN){
          ...
          if (result) {
              ...
              let _permission = JSON.stringify(result?.permissions);
              if(_module){
                  switch(req.method){
                      case 'GET':
                          if(_permission[_module]){
                              if(_permission[_module].read==true) resolve(false);
                              console.log(_permission[_module]);
                          }
                          break;
                      case 'POST':
                        if(_permission[_module]){
  ...
  async checkDBFilter(req,role_id=''){
    ...
    try{
      ...
      if(role_id != ''){
          ...
          if (result) {
              let _module = req?.headers['x-aio-module']??'';
              let _dbFilters = JSON.stringify(result?.db_filters);
